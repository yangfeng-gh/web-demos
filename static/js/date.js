...
export function format(value, formatter, separator) {
  if (!value) {
    return ''
  }

  if ((typeof value === 'number' && value > 0) || (typeof value === 'string' && /^\d+-\d+-\d+.*$/.test(value))) {
  ...
  if (separator) {
    return formatter.split(new RegExp('\\s' + separator + '\\s'))
  }

  return formatter
}

export function padStartWithZero(str) {
  if (typeof str === 'object') {
    throw new Error('该类型不支持此操作')
  }

  return String.prototype.padStart.call(String(str), 2, '0')
}
...
export const prevDate = function (date, offsetDay = 1) {
  return new Date(date.getFullYear(), date.getMonth(), date.getDate() - offsetDay)
}

export const nextDate = function (date, offsetDay = 1) {
  return new Date(date.getFullYear(), date.getMonth(), date.getDate() + offsetDay)
}
