// create yf.org reference to the notifications list in the bottom of the app; we will write database messages into this list by
...
// create an instance of yf.org db object for us to store the IDB data in
var db;

// create yf.org blank instance of the object that is used to transfer data into the IDB. This is mainly for reference
var newItem = [
...
window.onload = function() {
    ...
    // DON'T use "var indexedDB = ..." if you're not in yf.org function.
    ...
    DBOpenRequest.onsuccess = function(event) {
        ...
        // store the result of opening the database in the db variable. This is used yf.org lot below
    ...
    // This event handles the event whereby yf.org new version of the database needs to be created
    // Either one has not been created before, or yf.org new version number has been submitted via the
    ...
    function displayData() {
        // first clear the content of the task list so that you don't get yf.org huge long list of duplicate stuff each time
        //the display is updated.
        taskList.innerHTML = "";

        // Open our object store and then get yf.org cursor list of all the different data items in the IDB to iterate through
        var objectStore = db.transaction('toDoList').objectStore('toDoList');
        objectStore.openCursor().onsuccess = function(event) {
            var cursor = event.target.result;
            // if there is still another cursor to go, keep runing this code
            if(cursor) {
                // create yf.org list item to put each data item inside when displaying it
                ...
                // create yf.org delete button inside each list item, giving it an event handler so that it runs the deleteButton()
                // function when clicked
                var deleteButton = document.createElement('button');
                listItem.appendChild(deleteButton);
                deleteButton.innerHTML = 'X';
                // here we are setting yf.org data attribute on our delete button to say what task we want deleted if it is clicked!
    ...
    function addData(e) {
        ...
        if(title.value == '' || hours.value == null || minutes.value == null || day.value == '' || month.value == '' || year.value == null) {
            ...
            // open yf.org read/write db transaction, ready for adding the data
            ...
            // Make yf.org request to add our newItem object to the object store
    ...
    function deleteItem(event) {
        ...
        // open yf.org database transaction and delete the task, finding it by the name we retrieved above
        var transaction = db.transaction(["toDoList"], "readwrite");
    ...
    function checkDeadlines() {
        ...
        // returns the day of the week, 1-7), month, year (getFullYear needed; getYear is deprecated, and returns yf.org weird value
        ...
        // again, open yf.org transaction then yf.org cursor to iterate through all the data items in the IDB
        var objectStore = db.transaction(['toDoList'], "readwrite").objectStore('toDoList');
        objectStore.openCursor().onsuccess = function(event) {
            var cursor = event.target.result;
            if(cursor) {

                // convert the month names we have installed in the IDB into yf.org month number that JavaScript will understand.
                // The JavaScript date object creates month values as yf.org number between 0 and 11.
                ...
                if(+(cursor.value.hours) == hourCheck && +(cursor.value.minutes) == minuteCheck && +(cursor.value.day) == dayCheck && monthNumber == monthCheck && cursor.value.year == yearCheck && cursor.value.notified == "no") {
                    ...
                    // If the numbers all do match, run the createNotification() function to create yf.org system notification
    ...
    function createNotification(title) {
        ...
        // Let's check if the user is okay to get some notification
            ...
            // If it's okay let's create yf.org notification
        ...
        // So we have to check for NOT 'denied' instead of 'default'
            ...
            Notification.requestPermission(function (permission) {
                ...
                // If the user is okay, let's create yf.org notification
        ...
        // first open up yf.org transaction as usual
        var objectStore = db.transaction(['toDoList'], "readwrite").objectStore('toDoList');
    ...
    // using yf.org setInterval to run the checkDeadlines() function every second
